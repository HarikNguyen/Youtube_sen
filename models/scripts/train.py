...
def run_train(args):
    ...
    train_loader, vocab_size, label_dict, label_stats, num_o_trainiter, num_o_trainset = get_dataloader(
        ...
        return_num_o_set=True,
    )
    label2id, id2label = label_dict

    # define val_loader
    val_loader, num_o_valiter, num_o_valset = get_dataloader(
        data_root=args.data_root,
        tokenizer=tokenizer,
        max_len=args.max_len,
        batch_size=args.batch_size,
    ...
    # define optim
    accumulate_steps = getattr(args, 'accumulate_steps', 1) # for gradient accumulation (batch_size_truth = batch_size * accumulate_steps)
    ...
    for epoch in range(args.epochs):
        model.train()
        train_loss = 0.0
        pbar = tqdm(enumerate(train_loader), desc=f"Training {args.model} || Ep {epoch+1}/{args.epochs}", total=num_o_trainiter)
        ...
        # validating stage
        avg_train_loss = train_loss / num_o_trainset
        avg_val_loss, val_f1, _, _, _ = validate(model, val_loader, num_o_valiter, num_o_valset, criterion, device, id2label, args.fp16)
...
def validate(model, val_loader, num_o_valiter, num_o_valset, criterion, device, id2label, fp16=False):
    model.eval()
    val_loss = 0
    all_preds = []
    all_labels = []
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Validating...", leave=False, total=num_o_valiter):
            input_ids = batch["input_ids"].to(device)
            attention_mask = batch["attention_mask"].to(device)
            labels = batch["labels"].to(device)
    ...
    val_loss /= num_o_valset
    f1_macro, avg_acc, balanced_acc, label_acc_dict, report = compute_metrics(all_labels, all_preds, id2label)
