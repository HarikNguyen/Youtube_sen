############################################################################################
##                                TÙY BIẾN TÀI NGUYÊN (CPU & RAM)                         ##
############################################################################################
import os
import multiprocessing
# 1. TÙY CHỈNH CPU CORES
# Mặc định Polars sẽ dùng toàn bộ số core máy tính có. 
# Nên chừa lại 1-2 cores cho hệ điều hành hoạt động mượt mà.
TOTAL_CORES = multiprocessing.cpu_count()
TARGET_CORES = max(1, TOTAL_CORES - 2) 

# BẮT BUỘC: Phải set biến môi trường này TRƯỚC KHI import polars
os.environ["POLARS_MAX_THREADS"] = str(TARGET_CORES)

# 2. TÙY CHỈNH RAM (BỘ NHỚ)
# - RAM 8GB    : ~ 500_000
# - RAM 16GB   : ~ 1_000_000
# - RAM 32GB+  : ~ 2_500_000 hoặc cao hơn
BATCH_SIZE = 1_000_000

os.environ["POLARS_STRING_CACHE"] = "1"
