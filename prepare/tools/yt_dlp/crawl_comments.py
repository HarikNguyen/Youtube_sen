...
assert len(PROXIES_CONFIG) == len(COOKIE_FILES), "CẢNH BÁO: Số lượng Proxy phải bằng đúng số lượng file Cookie!"
...
class SessionManager:
    ...
    def get_session(self, index):
        with self.shuffle_lock:
            if time.time() - self.last_shuffled > SHUFFLE_INTERVAL:
                ...
                for i in range(len(self.proxies)):
                    print(f"  -> Proxy {i+1} ({self.proxies[i]['addr']}) được gắn với {self.cookies[i]}")
                print()
        ...
        proxy_url = f"http://{proxy_conf['user']}:{proxy_conf['pass']}@{proxy_conf['addr']}"
...
def get_transcript_safe(video_id, proxy_config, session_manager, max_retries=2):
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt < max_retries and any(x in err_msg for x in ["403", "proxy", "connection", "forbidden"]):
                print(f"[TRANSCRIPT RETRY] Cổng {proxy_config['addr']} bị chặn khi lấy sub. Đang xoay IP (Lần {attempt + 1}/{max_retries})...")
...
def process_single_video(video_task, thread_index, retry_count=1):
    ...
    category = "".join([c for c in video_task.get("category", "Uncategorized") if c.isalnum() or c==' ']).strip()
    channel = "".join([c for c in video_task.get("channel", "Unknown") if c.isalnum() or c==' ']).strip()
    ...
    try:
                ...
                if "comments" in info and info["comments"]:
                    for c in info["comments"]:
                        ...
                        if c_id not in dict_comments:
                            ...
                            formatted_date = datetime.datetime.fromtimestamp(raw_timestamp).strftime('%Y-%m-%d %H:%M:%S') if raw_timestamp else ""
        ...
        # Ghi file Metadata
        pd.DataFrame([metadata]).to_csv(os.path.join(temp_dir, "metadata.csv"), index=False, encoding="utf-8-sig")
        
        # Ghi file Comments (Đã lọc trùng Unique)
        unique_comments_list = list(dict_comments.values())
        if unique_comments_list:
            pd.DataFrame(unique_comments_list).to_csv(os.path.join(temp_dir, "comment.csv"), index=False, encoding="utf-8-sig")
        ...
        with open(os.path.join(temp_dir, "transcript.json"), "w", encoding="utf-8") as f:
            f.write(ts_data)
        ...
        if any(x in err_msg for x in ["403", "proxy", "connection", "forbidden"]):
            if retry_count > 0:
                print(f"[RETRY YT-DLP] Lỗi mạng/403. Đang xoay IP cho {proxy_config['addr']} và cào lại từ đầu...")
                session_manager.rotate_ip(proxy_config)
...
def scrape_youtube_fast(video_tasks_gen):
    ...
    with yt_dlp.YoutubeDL({"quiet": True}) as _dummy: pass

    max_threads = len(PROXIES_CONFIG) * MAX_THREADS_PER_PROXY
    print(f"[START] Đang chạy với {max_threads} threads (10 threads/Proxy) và {len(PROXIES_CONFIG)} Proxies...")
    ...
    def logger_worker():
        while is_running:
            ...
            with progress_lock:
                ...
                print(f"  > Progress     : {progress['success']} Suscess | {progress['skipped']} Skip | {progress['errors']} Error")
