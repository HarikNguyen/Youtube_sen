...
def process_parser(args):
    ...
    if args.expand_seed:
        ...
        for channel_seed_id in channel_seed_ids:
            ...
            discovered_channels = simulate_recommendation_engine(
                youtube_client, channel_seed_id
            )
        ...
        # Save results to CSV
        df = pd.DataFrame(
            expanded, columns=["channel_seed_id", "channel_id", "channel_title"]
        )
...
def main():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Generate initial seed data",
    )
    parser.add_argument(
        "--expand_seed",
        default=False,
        action="store_true",
        help="Expand data based on seed channels",
    )
    parser.add_argument(
        "--crawl", default=False, action="store_true", help="Start crawling process"
    )
    parser.add_argument(
        "--is_vn_channel",
        default=False,
        action="store_true",
        help="Filter for Vietnamese channels",
    )
    parser.add_argument(
        "--track_info",
        default=False,
        action="store_true",
        help="Track video statistics and comments",
    )
