...
def track_comment_counts(key_manager, input_path, ignore_path, output_path, chunk_size=10000):
    ...
    for chunk in reader:
        ...
        new_items_df['comment_count'] = new_items_df['video_id'].map(lambda x: all_stats.get(x, 'N/A'))
        
        mode = 'w' if is_first_chunk else 'a'
        header = is_first_chunk
        new_items_df.to_csv(output_path, index=False, mode=mode, header=header, encoding='utf-8-sig')
        ...
        if current_time - last_report_time >= report_interval:
            ...
            print(f"--- Time-based Update: {total_new_processed} new videos processed. (Elapsed: {int(elapsed_total)}s) ---")
            last_report_time = current_time

    print(f"--- Task Finished. Total new processed: {total_new_processed}. Results: {output_path} ---")
